...
Dim dblW0 As Double, dblWCap As Double, dblWEC As Double, dblDmax As Double, dblRmax As Double, dblDsum As Double
   ...
   Next i
   ' EC[]
   For i = 1 To im ' максимизируем выдачу только в часы дефицита
...
'   ub(im * 2 + 2) = MaxRealNumber ' если для расчетчика не требуется ограничение
   ...
   Next i
   ' раннее обнаружение несовместности условий задачи
   If dblSmin > 0 Then ' заряд невозможен, в том числе для покрытия нагрузки собственных нужд в режиме ожидания
      Debug.Assert 0
   End If
   If dblSmax < 0 Then ' разряд невостребован, ситуация не должна возникать при корректных ub для EC
      Debug.Assert 0
   End If
   ...
   Case 1, 5, 7, 8
      Debug.Print "Exit code = " & lRet  ' успешное завершение с иным критерием
...
Dim lRes As Long
Dim arr()
   ChDir ThisWorkbook.Path ' меняем текущий каталог на место расположения DLL (там же, где книга)
   ...
   dblP = 0 ' типовой расчет - нет потребления в режиме ожидания
