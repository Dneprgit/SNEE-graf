...
const BatteryInteractiveSection_qp = ({
  ...
  const paramsGroupButtonBase =
    'w-full min-h-[86px] text-sm text-center rounded-xl border-2 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md';
  const paramsGroupButtonActive = 'bg-blue-600 border-blue-700 text-white hover:bg-blue-700';
  const paramsGroupButtonInactive = 'bg-white border-blue-600 text-blue-700 hover:bg-blue-50';
  const solverButtonBase =
    'text-lg px-8 py-4 rounded-xl border-2 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md';
  const solverButtonActive = 'bg-blue-600 border-blue-700 text-white hover:bg-blue-700 ring-2 ring-blue-300 shadow-lg';
  const solverButtonInactive = 'bg-white border-blue-600 text-blue-700 hover:bg-blue-50';
      ...
      >
        ...
        <div className="card mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_260px] gap-6 items-stretch">
            {/* Группа 1: заводские параметры */}
            <div className="h-full flex flex-col">
              <div className={`mb-6 lg:mb-4 rounded-xl border-2 p-4 transition-all flex-1 ${
                isFactoryParamsActive ? 'border-blue-500 shadow-lg bg-blue-50/40' : 'border-slate-200 bg-white'
              }`}>
                <div className="flex items-center text-lg font-semibold text-gray-800">
                  <Info className="w-6 h-6 mr-2 text-primary-600" />
                  Заводские параметры СНЭЭ (параметры, задаваемые вручную)
                </div>
                

                {/*Сетка Интерактивных Параметров */}
                
                <div className="grid grid-cols-1 lg:grid-cols-[auto_1fr] gap-3 mb-4">
                  {/* Текущие параметры */}
                  ...
                  {/*Инфо панель, как использовать Интерактивные Параметры СНЭЭ */}
              ...
              <button
                type="button"
                onClick={onActivateFactoryParams}
                className={`${paramsGroupButtonBase} ${
                  isFactoryParamsActive ? paramsGroupButtonActive : paramsGroupButtonInactive
                } ${
                  isFactoryParamsActive ? 'ring-2 ring-blue-300' : ''
                }`}
              >
                Расчет с использованием заводских параметров СНЭЭ
              </button>
            </div>

            {/* Группа 2: оценочные параметры */}
            <div className="h-full flex flex-col">
              <div className={`grid grid-cols-1 gap-4 max-w-[260px] mt-6 lg:mt-0 rounded-xl border-2 p-4 transition-all flex-1 ${
                isOptimalParamsActive ? 'border-purple-500 shadow-lg bg-purple-50/40' : 'border-slate-200 bg-white'
              }`}>
                <div className="flex items-center text-lg font-semibold text-gray-800">
                  <Info className="w-6 h-6 mr-2 text-primary-600" />
                  Оценочные параметры СНЭЭ
                </div>
                <button
                  ...
                  title={optimalParams.power_out && optimalParams.capacity ? `Установить: Вход ${optimalParams.power_in.toFixed(2)} МВт, Выход ${optimalParams.power_out.toFixed(2)} МВт, Емкость ${optimalParams.capacity.toFixed(2)} МВтч` : 'Установить оценочные параметры'}
                >
                  <RotateCcw className="w-4 h-4 mr-2" />
                  {isLoadingOptimal ? 'Расчет...' : 'Установить оценочные параметры СНЭЭ как заводские'}
                </button>
                
                {isLoadingOptimal ? (
                  <div className="bg-gray-50 rounded-lg p-4 border-2 border-gray-300 text-center">
                    <div className="text-sm text-gray-600">Расчет оценочных параметров...</div>
                  </div>
                ) : optimalParams.power_out && optimalParams.capacity ? (
                  <>
                    <div className="bg-gradient-to-r from-red-50 to-red-100 rounded-lg p-3 border-2 border-red-300">
              ...
              <div className="max-w-[260px] w-full mt-4">
                <button
                  type="button"
                  onClick={onActivateOptimalParams}
                  disabled={!hasOptimalParams}
                  className={`${paramsGroupButtonBase} ${
                    isOptimalParamsActive ? paramsGroupButtonActive : paramsGroupButtonInactive
                  } ${
                    isOptimalParamsActive ? 'ring-2 ring-blue-300' : ''
                  } ${!hasOptimalParams ? 'opacity-50 cursor-not-allowed' : ''}`}
                  title={!hasOptimalParams ? 'Сначала выполните расчет оценочных параметров' : 'Использовать оценочные параметры для расчета графика'}
                >
                  Расчет с использованием оценочных параметров СНЭЭ
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-3">
          <button
            ...
            className={`${solverButtonBase} ${
              isLinprogActive ? solverButtonActive : solverButtonInactive
            } ${
          ...
          >
            {isLinprogCalculating ? (
              ...
              <>
                ...
                Рассчитать диспетчерский график работы СНЭЭ (решатель SciPy linprog)
              </>
            )}
          </button>
          ...
          <button
            ...
            disabled={!loadProfile || isCalculatingSchedule}
            className={`${solverButtonBase} ${
              isHighsActive ? solverButtonActive : solverButtonInactive
            } ${
              !loadProfile || isCalculatingSchedule ? 'opacity-50 cursor-not-allowed' : ''
            }`}
          >
