...
tmux send-keys -t $SESSION_NAME:0 'uvicorn main:app --reload --host 0.0.0.0 --port 8002' C-m
...
tmux send-keys -t $SESSION_NAME:2 'echo "Backend:  http://localhost:8002"' C-m
tmux send-keys -t $SESSION_NAME:2 'echo "Frontend: http://localhost:3002"' C-m
tmux send-keys -t $SESSION_NAME:2 'echo "API Docs: http://localhost:8002/docs"' C-m
