...
class EnergyStorageCalculator_qp:
    ...
    def _solve_qp_optimization(self, system_load: np.ndarray, dbl_n_in: float, 
        ...
        dbl_rmax_weight = dbl_d_weight / 25  # максимальный резерв мощности (25 = 24 часа + 1)
        ...
        # 6. Начальное приближение: x0 с учетом границ
        x0 = np.zeros(n)
        for i in range(n):
...
def calculate_optimal_parameters_qp(load_profile: List[float], efficiency: float) -> dict:
    ...
    for i in range(im * 3, im * 4):
        b[i] = dbl_d_weight
    b[im * 4] = dbl_dmax_weight  # вес для Dmax (дефицит мощности)
    b[im * 4 + 1] = dbl_nmax_weight  # вес для Nin (входная мощность)
    b[im * 4 + 2] = dbl_nmax_weight  # вес для Nout (выходная мощность)
    b[im * 4 + 3] = dbl_capacity_weight  # вес для C (емкость)
    ...
    # Границы для L[] - энергия в батарее
    for i in range(im):
        lb[i] = 0.0
    ...
    for i in range(im):
        lb[i + im] = 0.0
        ub[i + im] = max_real_number
    
    # Границы для CD[] - мощность разряда
    for i in range(im):
        lb[i + im * 2] = 0.0
        ub[i + im * 2] = max_real_number
    ...
    # 6. Начальное приближение, специфичное задаче
    x0 = [0.0] * n
    ...
    for i in range(im):
        ...
        x0[i + im] = dbl_val  # мощность заряда [CC]
        x0[i + im * 2] = dbl_val  # мощность разряда [CD]
    
    # Начальное приближение дефицит мощности
    for i in range(im):
        dbl_val = max(lb[i + im * 3], dbl_system_load[i] + x0[i + im] / efficiency - x0[i + im * 2])
