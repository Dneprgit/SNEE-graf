...
def calculate_optimal_parameters_qp(load_profile: List[float], efficiency: float) -> dict:
    ...
    im = 24  # количество часов
    n = im * 3 + 4  # 76 переменных: L[24] + CC[24] + CD[24] + Dmax + Nin + Nout + C
    k_eq = im  # 24 ограничения типа равенство: rL[24]
    k_ub = im * 4  # 96 ограничений типа неравенство: rD[24] + rC[24] + rNi[24] + rNo[24]
    ...
    c = np.zeros(n)
    # Веса для L[0..23], CC[0..23], CD[0..23] = 0 (уже установлены через zeros)
    c[im * 3] = dmax_weight  # Dmax - дефицит мощности
    c[im * 3 + 1] = nmax_weight  # Nin - входная мощность
    c[im * 3 + 2] = nmax_weight  # Nout - выходная мощность
    c[im * 3 + 3] = capacity_weight  # C - емкость
    ...
    if result.status > 0:
        error_msg = f"Optimization did not converge. Status: {result.status}, Message: {result.message}"
        print(f"Warning: {error_msg}")
        if result.status == 2:
            raise ValueError(f"Ограничения несовместны: {result.message}")
        elif result.status == 3:
            raise ValueError(f"Задача неограничена: {result.message}")
        elif result.status == 4:
            raise ValueError(f"Ошибка в алгоритме HiGHS: {result.message}")
    
    if result.x is None:
        raise ValueError(f"Оптимизация не вернула решение. Status: {result.status}, Message: {result.message}")
    ...
    system_with_enss_load_deficite = x[im * 3]  # Dmax
    n_in = x[im * 3 + 1]  # Nin
    n_out = x[im * 3 + 2]  # Nout
    capacity = x[im * 3 + 3]  # C
